...
open class ImagePickerController : UIViewController {
    ...
    public var selectedAssetsToIndexPath: [PHAsset: IndexPath] {
        get {
            let selectedIndexPaths = collectionView.indexPathsForSelectedItems ?? []
            return selectedIndexPaths.reduce(into: [PHAsset: IndexPath]()) { (dict, indexPath) in
                dict[asset(at: indexPath.row)] = indexPath
            }
        }
    }
    ...
    public var visibleAssetsToIndexPath: [PHAsset: IndexPath] {
        get {
            ...
            return visibleIndexPaths.reduce(into: [PHAsset: IndexPath]()) { (dict, indexPath) in
                dict[asset(at: indexPath.row)] = indexPath
            }
        }
    }
